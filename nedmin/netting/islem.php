...
if (isset($_POST['kullanicikaydet'])) {
	...
	//dışarıdan gelen kullanıcılardan zararlı script olasılığını düşünerek 
	//htmlsprecialchars parametresini kullanıyoruz
	$kullanici_adsoyad=htmlspecialchars($_POST['kullanici_adsoyad']); 
	$kullanici_mail=htmlspecialchars($_POST['kullanici_mail']); 
	...
	if ($kullanici_passwordone==$kullanici_passwordtwo) {
		//Şifrelerin 6 karakterden kısa olmasını istemediğimiz için kullanılan kod
...
// Başlangıç
			...
			if ($say==0) {
				//Eğer mail ile daha önceden kayıt olunmadıysa say 0 dönecektir.
				//Böylelikle daha önceden kayıt olunup olunmadığını görebiliriz.
				//md5 fonksiyonu şifreyi md5 şifreli hale getirir.
				$password=md5($kullanici_passwordone);
				//yetki 1 ise normal kullanıcı / 5 ise admin
				$kullanici_yetki=1;

			//Kullanıcı kayıt işlemi yapılıyor...
				$kullanicikaydet=$db->prepare("INSERT kullanici SET
					kullanici_adsoyad=:kullanici_adsoyad,
					kullanici_mail=:kullanici_mail,
					kullanici_password=:kullanici_password,
		...
		// Bitiş
			...
			//Şifre 6 karakterden küçükse buraya gönder
...
if (isset($_GET['slidersil'])){
	...
	if ($_GET['slidersil']=="ok") {
		//slider'ı silmek için gerekli olan kod
...
if (isset($_POST['sliderduzenle'])) {
	...
	//önceden eklenilen slider'ları tekrardan düzenleyebilmek için kullandığım kod
...
if (isset($_POST['sliderkaydet'])) {
	...
	//Panelden eklediğimiz sliderleri veritabanına kaydetmek
	//
	$uploads_dir = '../../dimg/slider';
	@$tmp_name = $_FILES['slider_resimyol']["tmp_name"];
	@$name = $_FILES['slider_resimyol']["name"];
...
if (isset($_POST['kullanicigiris'])) {
	...
	$kullanici_mail=htmlspecialchars($_POST['kullanici_mail']); 
	//Kullanıcı şifresi veritabanına md5 olarak kayıtlı olduğu için veritabanı sorgu sırasında md5 e çevirerek sorgulamalıyız.
	$kullanici_password=md5($_POST['kullanici_password']); 
...
if (isset($_POST['sepetekle'])) { // sepete eklenilen ürünler tabloya eklensin
...
if (isset($_GET['urun_onecikar'])) {  // ürünleri anasayfada öne çkan ürünler kısmında listelemek için
	if($_GET['urun_onecikar']=="ok"){ 
		$duzenle=$db->prepare("UPDATE urun SET
			urun_onecikar=:urun_onecikar
			WHERE urun_id={$_GET['urun_id']}");
		$update=$duzenle->execute(array(
			'urun_onecikar' => $_GET['onecikar']
		));
...
if (isset($_GET['yorum_onay'])) { // yapılan yorumlar panele düşer. gözükmesini isterseniz onayla butonuna basarak sayfada gösterebilirsiniz
	if($_GET['yorum_onay']=="ok") { 
		$duzenle=$db->prepare("UPDATE yorumlar SET
			yorum_onay=:yorum_onay
			WHERE yorum_id={$_GET['yorum_id']}");
		$update=$duzenle->execute(array(
			'yorum_onay' => $_GET['onay']
		));
...
if (isset($_POST['bankaekle'])) { // veritabanına tablo eklemek için
...
if (isset($_POST['bankaduzenle'])) { // veritabanına eklenen banka tablolarını düzenlemek için 
...
if (isset($_GET['bankasil'])){
	if ($_GET['bankasil']=="ok") {
		islemkontrol();
		//banka tablosu silmek için gerekli olan kod
...
if(isset($_POST['bankasiparisekle'])){
	...
	if($insert){
		echo $siparis_id=$db->lastInsertId(); // en son sipariş veren kişinin id'sini almak için gerekli olan kod bu kodu bir sonraki tabloya ekleyebilmek için kullanacağız.
		
		$kullanici_id=$_POST['kullanici_id']; // sepete ekleyen kullanıcının idsini alabilmek için
		...
		// siparişe verilen ürünler sipariş_detay tablosunda listelensin
		...
		if($insert){
			//ürünler sipariş_Detay tablosuna eklenirse sepetimi boşalt.
...
if(isset($_GET['sepetkaldir'])){
	islemkontrol();
	if ($_GET['sepetkaldir']=="ok") {
